...
The styling for which is given below. ~create-theme~ is a special mixin which generates all the required CSS variables according to the colour scheme map provided.
...
In Sass, all CSS custom properties (in the form ~--value: ...;~ ) do not evaluate the [[https://sass-lang.com/documentation/style-rules/declarations/#custom-properties][SassScript values set on them]]. It passes it directly to CSS. We have to use interpolation in the form of ~#{$var}~ to get around this issue.
** Dealing with icons
...
** Dealing with fonts
This was surprisingly the easiest part. Check out [[https://gwfh.mranftl.com/fonts][google-webfonts-helper]]. I simply picked out my fonts and installed it in ~woff2~ format. If you install fonts directly from Google then you get ~ttf~ files, though I didn't investigate the matter further. The added benefit of google-webfonts-helper is that it also gives you the CSS to copy and put in your website. Amazing.
* Conclusion
